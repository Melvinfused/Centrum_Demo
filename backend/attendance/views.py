...
class AttendanceCheckInView(generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if attendanceRecord.objects.filter(user=user, check_in_time__date=today).exists():
        ...
        if hasattr(user, "company"):  # e.g., user.company (FK)
            user_company = getattr(user, "company")
        elif hasattr(user, "companyProfile"):  # e.g., user.companyProfile
            user_company = getattr(user, "companyProfile")
        elif getattr(user, "company_id", None):  # direct FK id on user
            user_company = companyProfile.objects.filter(id=user.company_id).first()

        # If still not found, return useful error
        if not user_company:
            return Response({"error": "No company assigned to this user."}, status=status.HTTP_400_BAD_REQUEST)

        # Ensure company has coordinates
        if user_company.latitude is None or user_company.longitude is None:
            return Response({"error": "Assigned company does not have coordinates."}, status=status.HTTP_400_BAD_REQUEST)

        # Compute distance between provided coords and user's company coords
        distance_km = haversine(
            input_lat,
            input_lon,
            float(user_company.latitude),
            float(user_company.longitude)
        )
        ...
        if distance_km > 0.012:
            ...
            return Response({
                "error": "You are not at your assigned company location.",
                "distance_meters": round(distance_km * 1000, 2),
                "company": {
                    "id": user_company.id,
                    "name": user_company.company_name,
                    "latitude": user_company.latitude,
                    "longitude": user_company.longitude
                }
...
class AttendanceCheckOutView(generics.CreateAPIView):
    serializer_class = AttendanceRecordSerializer
    permission_classes = [permissions.IsAuthenticated]
    ...
    def create(self, request, *args, **kwargs):
        ...
        if attendanceRecord.objects.filter(user=user, check_out_time__date=today).exists():
